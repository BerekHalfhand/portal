...
(function withAngular(angular, navigator) {
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate) {
      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          var d = new Date(int_d - 1); //Workaround for chrome, the lamest

            var i
              , limitDate = new Date(year, month, 0).getDate()
              , firstDayMonthNumber = d.getDay()
  ...
  angular.module('720kb.datepicker', [])
               .directive('newDatepicker', ['$window', '$compile', '$locale', '$filter', '$interpolate', datepickerDirective]);
}(angular, navigator));
