  ...
  // <script>
    ...
    AutoComplete.prototype = {
        ...
        rteKeyDown: function (e) {
            ...
            //SPACE
            case 32:
              if (this.$dropdown === undefined || !$('.rte-autocomplete.dropdown-menu').is(':visible'))
                this.cleanUp(true);
              break;
            //UP ARROW
            case 38:
        ...
        show: function () {
            ...
            var ed = $('#'+this.editor.id);
            var position = { 'top': offset.top, 'left': offset.left };
            if ( ed.hasClass('mentions-on-top') ) position.top -= ed.outerHeight();

            this.$dropdown = $(this.renderDropdown())
                                .addClass( ed.hasClass('mentions-on-top')?'mentions-on-top':'' )
                                .css(position);
        ...
        render: function (item) {
            return '<li>' +
                        '<a href="javascript:;"><span>' + item[this.options.queryBy] + ', <span class="dark-grey">'+ item['WorkGroup'][0] +'</span></span></a>' +
                    '</li>';
        },
        ...
        init: function (ed) {
            ...
            function prevCharIsSpace() {
                ...
                var start = range.startOffset,
                      text = typeof range.startContainer.data !== 'undefined' ? (range.startContainer.data + ' ' || '') : '',
            ...
            ed.on('keydown', function (e) {
                if ((e.keyCode || e.which) === 8) {
                    setTimeout(function() {
                        var text = ed.selection.getRng(true).startContainer;
                        if (!prevCharIsSpace()
                        && text.parentElement
                        && text.parentElement.parentElement
                        && text.parentElement.parentElement.id
                        && text.parentElement.parentElement.id.indexOf("mention_") > -1) {
                            ed.dom.remove(text.parentElement.parentElement);
                        }
                    }, 0);
                }
            });
